...
class Twitternovelle < Sinatra::Base
  ...
  ENDE    = JSON.parse(IO.read(File.join(File.dirname(__FILE__), 'logs/', '6-ende.json') ))
  
  
  THEMES = [:våren, :lyst, :tre, :bar, :frisk, :ende]
  ...
  def initialize
    ...
    @session = {}
    # default track terms
    @session[:track_terms] = "#ruby"
    #@session[:tweets]      = []
    @session[:tweets]      = CURRENT['tweets'] || []
=begin
